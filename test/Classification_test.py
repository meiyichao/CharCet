#!/usr/bin/env python3
# ATAC-seq Model Evaluation Pipeline
# Evaluates a trained deep learning model on chromatin accessibility data
...
parser = argparse.ArgumentParser(description='ATAC-seq Model Evaluation Pipeline')
...
parser.add_argument('-m', '--model_path', required=True, 
                    help='Path to trained model file (class_model.pth)')
...
class ChromatinAccessibilityClassifier(nn.Module):
    """Deep learning model for predicting chromatin accessibility"""
    def __init__(self, num_gene, seq_len):
        super(ChromatinAccessibilityClassifier, self).__init__()
        ...
        self.fc4 = nn.Linear(64, 2)          # Output layer (classification)
...
def evaluate_model(model, genome, label, tf_gexp_norm, test_cell_idx, region_idx, output_dir, fold_idx):
    ...
    for cellid in test_cell_idx:
        ...
        for batch_start, batch_end in batch_indices:
            ...
            Y_pred_0_prob = Y_pred[:, 0].numpy()
            Y_pred_1_prob = Y_pred[:, 1].numpy()
            class_pred = Y_pred.argmax(dim=1).numpy()
            ...
            for i in range(batch_size):
                results.append({
                    ...
                    'cellid': cellid,
                    'Y_pred_0_prob': Y_pred_0_prob[i],
                    'Y_pred_1_prob': Y_pred_1_prob[i],
    ...
    output_file = os.path.join(output_dir, f'{fold_idx}_class.csv')
    results_df.to_csv(output_file, index=False)
    
    # Print completion message
    end_time = time.time()
    duration = (end_time - start_time) / 60
    print(f"Evaluation completed in {duration:.2f} minutes")
