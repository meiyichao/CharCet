...
# Divide real labels into different categories based on value ranges
def assign_label_range(label):
    if label >= 0 and label < 0.1:
        return 0
    elif label >= 0.1 and label < 2.5:
        return 1
    elif label >= 2.5:
        return 2
    else:
        return -1  # Assign -1 for values outside the defined ranges
...
def model_training(model, batch_size, epochs):
    ...
    for _, labels in train_samples:
        for label in labels:
            assigned_label = assign_label_range(label)
            if assigned_label != -1:
                class_counts[assigned_label] += 1
    ...
    # Assign weights to each sample based on category weights
    weights = []
    for data, labels in train_samples:
        for label in labels:
            assigned_label = assign_label_range(label)
            if assigned_label != -1:
                weights.append(class_weights[assigned_label].item())
        
    #sampler = WeightedRandomSampler(torch.tensor(weights), num_samples=len(train_dataset), replacement=True)
    ...
    #sampler = WeightedRandomSampler(weights,num_samples=int(len(train_dataset)*ratio), replacement=True)
    ...
    train_dataloader = DataLoader(subset, batch_size=batch_size, shuffle=True,num_workers=8,drop_last=True)
    ...
    for epoch in range(1,epochs+1):
        ...
        if valid_loss < best_validation_loss - min_delta:
            ...
            early_stopping_counter = 0
            torch.save(model,'%s/data/checkpoint_regress/model.pth'%(DPATH))
        else:
...
if  __name__ == "__main__" :
    DPATH="/home/ycmei/model_demo" 
    genome = Fasta("%s/data/union.peaks.pad1k.fa"%DPATH) 
    label_file='%s/data/union.peaks.labels.regress.txt'%DPATH
    tf_gexp_file = '%s/data/final_exp_matrix.txt'%DPATH
    ...
    if ratio<0 or ratio>1: 
        ...
        sys.exit()    
    cellid_info = open('%s/data/Leave_one_out_cross_validation.txt'%DPATH).readlines()[fold_idx] 
